...
export const userValidation = {
  register: Joi.object({
    ...
    role: Joi.string().valid('admin', 'stylist', 'customer').optional().default('customer')
  }),
...
export const serviceValidation = {
  create: Joi.object({
    ...
    category: Joi.string().valid(
      'haircut', 'beard_trim', 'hair_wash', 'styling', 'coloring', 'treatment', 'package'
    ).required().messages({
...
export const bookingValidation = {
  ...
  update: Joi.object({
    ...
    appointmentTime: Joi.string().pattern(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional().messages({
      'string.pattern.base': 'Please provide a valid time in HH:MM format'
    }),
...
export const stylistValidation = {
  create: Joi.object({
    ...
    specialties: Joi.array().items(Joi.string().min(2).max(100)).min(1).required().messages({
    ...
    portfolio: Joi.array().items(Joi.string().uri()).max(10).optional().messages({
